...
window.renderStatistics = function (ctx, names, times) {
  ...
  var renderCloud = function (x, y, color) {
    ...
    var cloudMarginY = 10;
    ctx.beginPath();
    moveTo(x + cloudMarginX, y + cloudMarginY);
  ...
  for (i = 0; i < results.length; i++) {
    renderText(results[i], CLOUD_WIDTH / 2 + CLOUD_X - Math.round((ctx.measureText(results[i])).width / 2), CLOUD_Y + (TEXT_HEIGHT + GAP) * (i + 1));
  }
  ...
  var renderStatistics = function (x, y, name, time) {
    ctx.fillStyle = getBarColor(name);
    var barHeight = time * MAX_BAR_HEIGHT / maxTime;
    ctx.fillRect(x, y - barHeight, BAR_WIDTH, barHeight);
    renderText(Math.floor(time), x, y - barHeight - MIN_GAP);
  };
  // Draw game statistics for each player
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    renderText(names[i], barX, textY);
    renderStatistics(barX, textY - TEXT_HEIGHT, names[i], times[i]);
  }
  return 0;
};
